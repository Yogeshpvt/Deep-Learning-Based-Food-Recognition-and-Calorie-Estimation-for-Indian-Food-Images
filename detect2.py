...
ROOT = 'yolov5'
#source_path = 'C:/Users/YOGESH V/Desktop/yolov5-master/data/images/original_video.avi'
#source_path = '{dataset.location}/test/images/'
...
def run(
        weights=ROOT / 'runs/train/exp33/weights/best.pt',  # model path or triton URL
        source= ROOT / 'data/images',  #source_path ,  # file/dir/URL/glob/screen/0(webcam)
        data=ROOT / 'data/road_sign_data.yaml',  # dataset.yaml path
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            if names[c] not in food_frequency:
                                food_frequency[names[c]] = 1
                            else:
                                food_frequency[names[c]] += 1
                            total_calories = 0
                            for food, frequency in food_frequency.items():
                                if food in data['Name']:
                                    index = data['Name'].index(food)
                                    total_calories += data['Calories'][index] * frequency
                            
                            LOGGER.info(f'Total Calories from the Image: {total_calories} kcal')
                        if save_crop:
    ...
    ############        
            ...
            #GG = np.array(G)
            #print(FF) 
            #np.save('FILE_F.npy',FF)
            #np.save('FILE_G.npy',GG)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp37/weights/best.pt', help='model path or triton URL')
    parser.add_argument('--source', type=str,default=ROOT / 'data/images', help='file/dir/URL/glob/screen/0(webcam)') #default=source_path
    parser.add_argument('--data', type=str, default=ROOT / 'data/road_sign_data.yaml', help='(optional) dataset.yaml path')
